...
gee2_coefficients<-as.data.frame(summary(gee_out7)$coefficients) %>%
  ...
  dplyr::mutate(one_sided_pval=ifelse(Estimate>0, pval/2, (1-pval/2)),
                ...
                Std.Error_odds=exp(Estimate+Std.err+ctrl1)-exp(Estimate-Std.err+ctrl1),
...
p5<-ggplot(gee2_coefficients_plot %>%
  ...
  scale_color_manual(values=c("grey","#6B0504"), name="Adjusted p<0.05")+coord_flip()+
  ylab("Odds ratio of rose freshness compared to water")+
  xlab("Compound")+
  theme(legend.position = "top")
ggsave("figures/mod2_outcome.pdf", height=4.5, width=6)
...
gee2_coefficients<-as.data.frame(summary(gee_out7)$coefficients) %>%
  ...
  dplyr::mutate(one_sided_pval=ifelse(Estimate>0, pval/2, (1-pval/2)),
                ...
                significant_higher=ifelse(p_adjusted<0.05, T, F),
                Estimate_odds=exp(Estimate+ctrl1),
                probfit=Estimate_odds/(1+Estimate_odds),
                comp_water= probfit/(exp(ctrl1)/(1+exp(ctrl1))),
                Std.Error_prob=exp(Estimate+ctrl1+Std.err)/(1+exp(Estimate+ctrl1+Std.err))-
                  exp(Estimate+ctrl1-Std.err)/(1+exp(Estimate+ctrl1-Std.err)))
#                odds_ratio =Estimate_odds/exp(ctrl1))
gee2_coefficients$Estimate_odds[1]=exp(gee2_coefficients$Estimate[1])
gee2_coefficients$probfit[1]= exp(ctrl1)/(1+exp(ctrl1))
gee2_coefficients$comp_water[1]= 1

gee2_coefficients_plot=gee2_coefficients
gee2_coefficients_plot$predictor[1]="day:compound1"
p5<-ggplot(gee2_coefficients_plot %>%
         mutate(compound=factor(gsub("day:compound|","",predictor), levels=1:15)), 
       aes(x=compound, y=comp_water, color=p_adjusted<0.05))+
  geom_hline(yintercept=1, linetype="dashed")+
  geom_errorbar(aes(ymin=comp_water - Std.Error_prob, ymax=comp_water +Std.Error_prob), width=0.2)+geom_point(size=1.8)+theme_bw()+
  scale_color_manual(values=c("grey","#6B0504"), name="Adjusted p<0.05")+coord_flip()+
...
x<-glht(gee_out7,  linfct=c("day:compound6 - day:compound14==0"))
        
mtests1<-wrap_glht_output(x)
print(xtable(mtests1,digits=c(1,2,3,3,4)), file="output_xtables/mod2_contrast.tex")
