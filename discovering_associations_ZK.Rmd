...
glmer_out_bn <- glmer(fresh ~ compound + day + compound*day + species + garden + (1|rater) + (1|subplotID/bushID), family=binomial, data=d_full)
...
lme_out <- nlme::lme(width ~ compound + type + garden + days + compound*days, data=dataG_long, random = ~1|subplot/garden, na.action=na.omit)
```
...
ggplot(both_predictions, aes(x=Estimate, y=Value, color=significant_higher, shape=significant_lower, alpha=significant_in_either))+
...
-recheck nesting formulation
-model 2: a bit weird still?? Convergence,  Can we get a hint from professors
-write an email: is width increase a good thing? 
